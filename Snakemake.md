...
- A common error here would be to think that you can use glob to specify the input for the rule generate_coverage. Indeed glob allows to list all files in a folder ending with a particular extension. But that won't work. Snakemake needs to know about what sort of input this rules requires, and that needs to be made clear before any file is generated, before any task starts. So input will be evaluated before any bam file is generated and the result would be an empty list which will then have jgi_summarize_bam_contig_depths run on an empty folder.
...
-   As a snakemake grow bigger, ambiguity in rules may pop up : 2 rules with the same output, so 2 possible rule to create the same result. Which one should snakemake use? To solve these issues, you need to restrict your rules by making them less universal. For instance if you have 2 rules generating {genome}.gff, you can solve the ambiguity by either using a specific path: prodigal/{genome}.gff, or a specific filename: /{genome}_prodigal.gff. You can also constrain wildcards or specify a priority of rules (see the doc).
-   Snakemake only keeps track of files specified in "input" and "output". Snakemake will remove any output file generated by a task which failled. Non tracked files are not touched/monitored, which may be an issue if you are unaware that the task failled.
...
-   Snakemake will resolve the sequence of rules execution before starting --> if you don't know beforehand the number of files generated, it makes things more complicated. You can: 
	...
	- Use a more complex snakemake concept: checkpoints (see the doc). 
	- Stop using snakemake to monitors theses files. Don't refers to them explicitly in input/output. Instead create emtpy "flag" at the end of a rule execution and have snakemake take that as input/output. This however can create issues with in conjunction with the touch command.
- When your number of tasks increase over 10000 snakemake slows down from the sheer quantity of files/tasks to keep track of. In these case, it is better to use a hierachical approach: snakemake calling a snakemake or using gnu parallel to call multiple snakemakes. Or just create your own script for a succession of simples steps.
